...
from .models import jiankangyujing, popupremind, users, xinliyisheng, xinqingriji
...
def _can_read_diary(request, data):
    if not data:
        return False
    tablename, params = _token_info(request)
    if tablename in ('users', 'xinliyisheng'):
        return True
    if tablename == 'yonghu':
...
def _create_diary_keyword_warning(diary):
    if not diary:
        return {"created": False, "recipientCount": 0, "keywords": []}
    ...
    if jiankangyujing.objects.filter(yujingtixing__contains=marker).exists() or popupremind.objects.filter(
        ...
        return {"created": False, "recipientCount": 0, "keywords": keywords}

    now = datetime.datetime.now()
    keyword_text = "、".join(keywords)
    title_text = "{}：命中关键词 {} {}".format(DIARY_WARNING_TITLE, keyword_text, marker)
    suggestion = "用户心情日记中出现高风险词：{}。请管理员或心理医生及时查看日记内容并主动联系用户，必要时进行危机干预。".format(keyword_text)
    preview = _truncate_text(diary.rijineirong, 180)
    brief = "{} {}".format(DIARY_WARNING_TITLE, marker)
    content = (
        "{}\n"
        "用户账号：{}\n"
        "用户姓名：{}\n"
    ...
    recipients = _diary_warning_recipients()
    ...
    return {"created": True, "recipientCount": len(recipients), "keywords": keywords}
