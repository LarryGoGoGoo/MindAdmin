...
def news_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        __foreEndList__ = getattr(news, "__foreEndList__", None)
        __foreEndListAuth__ = getattr(news, "__foreEndListAuth__", None)

        #authSeparate
        __authSeparate__ = getattr(news, "__authSeparate__", None)
    ...
    #非管理员权限的表,判断当前表字段名是否有userid
        ...
        __authTables__ = getattr(news, "__authTables__", None)
...
def news_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        #浏览点击次数
        __browseClick__ = getattr(news, "__browseClick__", None)
