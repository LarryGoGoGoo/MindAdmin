...
def exampaper_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        __foreEndList__ = getattr(exampaper, "__foreEndList__", None)
        __foreEndListAuth__ = getattr(exampaper, "__foreEndListAuth__", None)

        #authSeparate
        __authSeparate__ = getattr(exampaper, "__authSeparate__", None)
    ...
    #非管理员权限的表,判断当前表字段名是否有userid
        ...
        __authTables__ = getattr(exampaper, "__authTables__", None)
...
def exampaper_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= exampaper.createbyreq(exampaper,exampaper, req_dict)
...
def exampaper_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        #浏览点击次数
        __browseClick__ = getattr(exampaper, "__browseClick__", None)
