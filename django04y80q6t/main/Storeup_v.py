...
def storeup_list(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        __foreEndList__ = getattr(storeup, "__foreEndList__", None)
        __foreEndListAuth__ = getattr(storeup, "__foreEndListAuth__", None)

        #authSeparate
        __authSeparate__ = getattr(storeup, "__authSeparate__", None)
    ...
    #非管理员权限的表,判断当前表字段名是否有userid
        ...
        __authTables__ = getattr(storeup, "__authTables__", None)
...
def storeup_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        #浏览点击次数
        __browseClick__ = getattr(storeup, "__browseClick__", None)
        ...
        if username:
            ...
            for column in ('username', 'name'):
                if column in columns:
                    record = storeup.objects.filter(**{column: username}).first()
                    if record:
                        break
