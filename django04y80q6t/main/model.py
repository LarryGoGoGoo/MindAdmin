...
from util.security import normalize_password_fields, public_record
...
class BaseModel(models.Model):
    ...
    def __Page(self, model, params, request, q):
        '''
        刷表专用
        http://ip:port/Django04y80q6t/${tableName}/page
        page 当前页
        pagesize 每页记录的长度
        ...
        for k, v in params.items():
            if "%" in str(v):
                ...
                if fuzzy_key != None:
                    fuzzy_filters["{}__icontains".format(fuzzy_key)] = fuzzy_val
            else:
        ...
        if sort != None or __sort__ != None:
            ...
            for index, value in enumerate(sort.split(",")):
                if order != None and order.split(",")[index] == 'desc':
                    order_by_fields.append("-{}".format(value))
                else:
                    order_by_fields.append(value)

        datas = model.objects.filter(**condition).filter(q)
        if fuzzy_filters:
            datas = datas.filter(**fuzzy_filters)
        if per:
            if start_time is not None and end_time is not None:
                datas = datas.filter(**{"{}__range".format(per): [start_time, end_time]})
            elif start_time is not None:
                datas = datas.filter(**{"{}__gte".format(per): start_time})
            elif end_time is not None:
                datas = datas.filter(**{"{}__lte".format(per): end_time})
        if order_by_fields:
            datas = datas.order_by(*order_by_fields)
        datas = datas.all()
    ...
    def __CreateByReq(self, model, params):
        ...
        '''
        # 密码强度校验
        from util.security import validate_password_strength
        for pw_field in ("password", "mima"):
            pw = params.get(pw_field)
            if pw:
                valid, err_msg = validate_password_strength(pw)
                if not valid:
                    return err_msg

        params = normalize_password_fields(params)

        if model.__tablename__ != 'users':
            import uuid
            params['id'] = uuid.uuid4().int >> 64
    ...
    def __GetBetweenParams(self, model, columnName, params):
        ...
        try:
            if type(remindstart) is str or type(remindend) is str:
                remindstart = float(remindstart)
                remindend = float(remindend)
        except Exception:
            pass
    ...
    def __UpdateByParams(self, model, params):
        ...
        for k, v in newParams.items():
            if k in column_list:
                paramss[k] = v
        for password_field in ("password", "mima"):
            if paramss.get(password_field) in ("", None):
                paramss.pop(password_field, None)
            else:
                # 密码强度校验
                from util.security import validate_password_strength
                valid, err_msg = validate_password_strength(paramss.get(password_field))
                if not valid:
                    return err_msg
        paramss = normalize_password_fields(paramss)
        try:
    ...
    def to_list(self, datas,model):
        ...
        try:
            ...
            pass
        return [public_record(item) for item in dataList]
