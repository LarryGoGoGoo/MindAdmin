...
def index(request):
    if request.method in ["GET", "POST"]:
        ...
        logging.getLogger("django.request").debug("=================>index")
...
def test(request, p1):
    if request.method in ["GET", "POST"]:
        msg = {"code": 200, "msg": "success", "data": []}
        logging.getLogger("django.request").debug("=================>index  %s", p1)
        return JsonResponse(msg)
...
def check_suffix(filelName,path1):
    ...
    target_path = Path(path1).resolve()
    if project_root not in target_path.parents and target_path != project_root:
        return HttpResponse("非法文件路径", status=403)
    try:
        image_data = open(str(target_path), "rb").read()
    except Exception:
        image_data = "no file"
    content_type = mimetypes.guess_type(str(filelName))[0]
    if content_type:
...
def admin_lib2(request, p1, p2):
    if request.method in ["GET", "POST"]:
        ...
        return check_suffix(_route_filename(request),path1)
        # try:
...
def layui2(request, p1, p2):
    if request.method in ["GET", "POST"]:
        ...
        logging.getLogger("django.request").debug("path1=%s", path1)
