	...
	<view :style='{"minHeight":"100%","width":"100%","padding":"0 24rpx 0 24rpx","position":"relative","background":"#fff","height":"auto"}' v-if="showType==1">
		<form :style='{"width":"100%","padding":"24rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 2rpx 0","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<xia-editor ref="content" :style='{"minHeight":"300rpx","width":"100%","padding":"0","color":"#666","background":"#fff","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxinlizhishi`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`discussxinlizhishi`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`discussxinlizhishi`, this.ruleForm);
				}else{
					oet = await this.$api.add(`discussxinlizhishi`, this.ruleForm);
				}
