	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" @scroll="scrollChange">
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
            <view class="search-form round">
                ...
                <input v-model="searchForm.name" type="text" placeholder="心理测评名称" ></input>
            </view>
            <view class="action">
			...
			<view class="list">
				<view @tap="onDetailTap(item)" v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
					...
					<view class="listmr">
						...
						<view class="colb f24 lh35">测评时长：{{item.time}}分钟</view>
						<view class="flex flex-align-center mt20">
							<text class="colb f25">
								<button class="cu-btn bg-green shadow">开始测评</button>
							</text>
						</view>
					</view>
				</view>
