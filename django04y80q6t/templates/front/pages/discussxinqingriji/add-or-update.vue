	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussxinqingriji`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`discussxinqingriji`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`discussxinqingriji`, this.ruleForm);
				}else{
					oet = await this.$api.add(`discussxinqingriji`, this.ruleForm);
				}
