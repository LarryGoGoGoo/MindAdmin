...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussxinqingrijiCleanType')
			if(cleanType){
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussxinqingriji', this.id);
		...
		methods: {
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussxinqingriji', this.id);
				let reg=new RegExp('http://localhost:8080/django04y80q6t/upload','g')//g代表全部
				if(res.data.content){
				...
				if(type==2){
					...
					await this.$api.update('discussxinqingriji',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussxinqingriji', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussxinqingriji',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussxinqingriji/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussxinqingriji',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussxinqingriji',that.detail)
							setTimeout(()=>{
								that.mescroll.num = 1
