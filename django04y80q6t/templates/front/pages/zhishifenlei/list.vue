			...
			<view :style='{"minHeight":"100%","alignContent":"flex-start","padding":"120rpx 24rpx 240rpx 24rpx","alignItems":"flex-start","flexWrap":"wrap","background":"#ffffff","display":"block","width":"100%","position":"relative","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"padding":"20rpx 0 0 0","margin":"0","background":"none","display":"flex","width":"100%","position":"relative","height":"auto"}'>
					<view  :style='{"border":"none","margin":"0","borderRadius":"20rpx","flex":"1","background":"none","lineHeight":"80rpx","position":"relative","height":"80rpx"}' class="search-form round">
						...
						<input  :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#8262c5","borderRadius":"40rpx","background":"#8262c520","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.zhishifenlei" type="text" placeholder="知识分类" ></input>
					</view>
					...
					<view class="uni-product-list" :style='{"padding":"0","margin":"0px 0 0 0","flexWrap":"wrap","flex":"auto","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"border":"2rpx solid #8262c530","boxShadow":"none","padding":"16rpx","margin":"0 0 30rpx 0","overflow":"hidden","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view :style='{"width":"100%","padding":"0","justifyContent":"space-between","display":"flex","height":"auto","order":"11"}'>
								<view :style='{"padding":"8rpx 12rpx","background":"none","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('zhishifenlei','修改')) || (!userid && isAuthFront('zhishifenlei','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"padding":"8rpx 12rpx","background":"none","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('zhishifenlei','删除')) || (!userid && isAuthFront('zhishifenlei','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"0","justifyContent":"space-between","display":"flex","height":"auto","order":"11"}'>
								<view :style='{"padding":"8rpx 12rpx","background":"none","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('zhishifenlei','修改')) || (!userid && isAuthFront('zhishifenlei','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#42a5f9","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","whiteSpace":"nowrap","color":"#42a5f9","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"padding":"8rpx 12rpx","background":"none","justifyContent":"center","display":"flex"}' v-if="(userid && isAuth('zhishifenlei','删除')) || (!userid && isAuthFront('zhishifenlei','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"24rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"24rpx","lineHeight":"1","whiteSpace":"nowrap","color":"#ff0000","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #dddddd","padding":"0 30rpx","margin":"0 10rpx 0 0","color":"#fff","bottom":"0","right":"0","outline":"none","borderRadius":"20rpx","background":"#b295f7","width":"auto","lineHeight":"70rpx","fontSize":"28rpx","position":"relative","height":"70rpx","zIndex":"1"}' v-if="userid && isAuth('zhishifenlei','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #dddddd","padding":"0 30rpx","margin":"0 10rpx 0 0","color":"#fff","bottom":"0","right":"0","outline":"none","borderRadius":"20rpx","background":"#b295f7","width":"auto","lineHeight":"70rpx","fontSize":"28rpx","position":"relative","height":"70rpx","zIndex":"1"}' v-if="!userid && isAuthFront('zhishifenlei','新增')" class="add-btn" @click="onAddTap()">新增</button>
			</view>
		</view>
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await that.$api.del('zhishifenlei', JSON.stringify([id]));
							that.$utils.msg('删除成功');
							that.hasNext = true
							// 重置分页参数页数为1
							that.search()
