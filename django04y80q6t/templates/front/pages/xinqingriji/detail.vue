		...
		<view class="container" :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
			...
			<view :style='{"width":"100%","padding":"24rpx","background":"none","height":"auto"}' class="detail-content">
				...
				<view :style='{"margin":"0 0 4rpx 0","borderColor":"#eeeeee","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="detail-list-item title">
					<view :style='{"padding":"0 20rpx 0 0","whiteSpace":"nowrap","color":"#333","textAlign":"right","width":"auto","lineHeight":"80rpx","fontSize":"28rpx"}' class="lable">日记标题：</view>
					<view :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#333"}' class="text" >{{detail.rijibiaoti}}</view>
				</view>
				...
				<view :style='{"width":"100%","display":"flex"}'>
					<view :style='{"width":"100%","margin":"0 0 24rpx 0","background":"#64558020","justifyContent":"center","display":"flex","height":"auto"}' class="detail-list-item" v-if="!thumbsupFlag&&!crazilyFlag" @tap="zan">
					...
					<view :style='{"width":"100%","margin":"0 0 24rpx 0","background":"#4D357B30","justifyContent":"center","display":"flex","height":"auto"}' class="detail-list-item" v-if="!thumbsupFlag&&!crazilyFlag" @tap="cai">
				...
				<view class="detail-list-item" :style='{"margin":"0 0 4rpx 0","borderColor":"#eeeeee","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"padding":"0 20rpx 0 0","whiteSpace":"nowrap","color":"#333","textAlign":"right","width":"auto","lineHeight":"80rpx","fontSize":"28rpx"}'>日记内容：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"#333"}'>{{detail.rijineirong}}</view>
				</view>
				...
				<view class="time-content" :style='{"width":"100%","margin":"0 0 24rpx 0","height":"auto"}'>
					<view class="comoment-header" :style='{"boxShadow":"none","borderRadius":"12rpx","background":"#8262c520","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
						<view :style='{"padding":"0 24rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#8262c5"}'>评论</view>
						<view :style='{"padding":"0 20rpx 0 20rpx","borderRadius":"0 12rpx 12rpx 0","background":"linear-gradient(126deg, #b295f7 2.814%,#8262c5 97.456%)","display":"flex"}' @click="onCommentTap" class="btn-comment-content" style="display: flex;align-items: center;">
					...
					<view :style='{"border":"2rpx solid #8262c520","boxShadow":"none","padding":"24rpx","margin":"24rpx 0 0 0","borderRadius":"20rpx","background":"#8262c506","width":"100%","position":"relative","height":"auto"}' v-for="(item,index) in commentList" v-bind:key="index" class="cu-item comment-item">
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discussxinqingrijiCleanType')
			if(cleanType){
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('xinqingriji', this.id);
				let reg=new RegExp('http://localhost:8080/django04y80q6t/upload','g')//g代表全部
				this.detail = res.data;
				this.title = this.detail.rijibiaoti
			}
		},
		destroyed: function() {
		...
		methods: {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('xinqingriji',_this.detail)
			...
			async init(type=1){
				...
				let res = await this.$api.info('xinqingriji', this.id);
				let reg=new RegExp('http://localhost:8080/django04y80q6t/upload','g')//g代表全部
				this.detail = res.data;
				...
				// 轮播图片
				this.swiperList = this.detail.rijitupian ? this.detail.rijitupian.split(",") : [];
				...
				if(type==2){
					...
					await this.$api.update('xinqingriji',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussxinqingriji', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussxinqingriji',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussxinqingriji/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussxinqingriji',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('xinqingriji',that.detail)
							setTimeout(()=>{
								that.mescroll.num = 1
